...
var controlKeys = [38,32,39,40,37];
var downKeys = [];
$(document).keydown(function(e) {
//	alert(e.which);
	if (controlKeys.indexOf(e.which) > -1){
		e.preventDefault();
		if (downKeys.indexOf(e.which) < 0){
			send_ctrl("dn "+e.which);
...
//	alert(e.which);
	send_ctrl("up "+e.which);
	downKeys.pop(e.which);
});
//
// Ajax Status
//
...
ajax_status.onreadystatechange = function() {
  lineCommands = (ajax_status.responseText);
  lineCommands = lineCommands.split("\n");
  if(ajax_status.readyState == 4 && ajax_status.status == 200) {
    if (lineCommands.length > 1) {
        //alert(lineCommands[0]);
	if (lineCommands[1] == "lt_off"){
      		document.getElementById("light_button").disabled = false;
    		document.getElementById("light_button").value = "Headlight On";
   		document.getElementById("overlay_light_button").src = "light.png";
  		document.getElementById("overlay_light_button").onclick = function() {send_func("lt_on");};
	} else if (lineCommands[1] == "lt_on") {
      		document.getElementById("light_button").disabled = false;
    		document.getElementById("light_button").value = "Headlight Off";
   		document.getElementById("overlay_light_button").src = "light_on.png";
		document.getElementById("overlay_light_button").onclick = function() {send_func("lt_off");};
	}
	//updatePreview();
    	(ajax_status.responseText) = lineCommands[0];
    }
    //alert(lineCommands[0]);	
    if(lineCommands[0] == "ready") {
...
function send_func (cmd) {
  ajax_cmd.open("GET","cmd_func.php?cmd="+cmd,true);
  ajax_cmd.send();
}
var ajax_ctrl_cmd;
if(window.XMLHttpRequest) {
  ajax_ctrl_cmd = new XMLHttpRequest();
} else {
  ajax_ctrl_cmd = new ActiveXObject("Microsoft.XMLHTTP");
}

function send_ctrl (cmd) {
  ajax_ctrl_cmd.open("GET", "control_pipe.php?cmd="+cmd,true);
  ajax_ctrl_cmd.send();
}
