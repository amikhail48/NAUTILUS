...
void LSM6DS3_Setup(int pfd){
    ...
    i2cWriteByteData(pfd,0x0e,0x08); 
    i2cWriteByteData(pfd,0x0a,0x31);
    
    LSM6DS3_EmptyFIFO(pfd);
    i2cWriteByteData(pfd,0x0a,0x30);
    i2cWriteByteData(pfd,0x13,0x00);
    i2cWriteByteData(pfd,0x11,0b01100100);
    i2cWriteByteData(pfd,0x10,0b01000000);
    i2cWriteByteData(pfd,0x08,0b00001100); //gyro no decimation, xl decimation 4
    i2cWriteByteData(pfd,0x07,0x00);
    i2cWriteByteData(pfd,0x06,0x90); //set FTH
    i2cWriteByteData(pfd,0x0a,0b00110001);// ODR=416, CONTINUOUS MODE
...
void *runCollection(void *bp){
	...
	if (pArgs->BLOCK_READY==1) {
		...
		if ( ((fifoStatus)&(0x6000))==  0 )
			...
			while((i2cReadByteData(pfd,0x3A) && (0x10))==0){
