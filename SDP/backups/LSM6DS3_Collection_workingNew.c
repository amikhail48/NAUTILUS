			...
			*/
			...
			if (CAL_MODE) {
				...
				if (cal_count == CALIB_SAMPLE_COUNT){
					/*if (pArgs->CAL_COMPLETE == 0){
						offset_xl_z = LSM6DS3_XLG*sum_xl_valz/(double)(4.0*32768.0*CALIB_SAMPLE_COUNT);
						pArgs->CAL_COMPLETE = 2;
					...
					//	avg_xl_valz = LSM6DS3_XLG*(sum_xl_valz/((double)CALIB_SAMPLE_COUNT*4.0*32768.0))-offset_xl_z;
				...
				if ( ((meas_angle_x < -90) && (kalAngleX > 90)) || ((meas_angle_x > 90) && (kalAngleX < -90)) ){
					...
					kalAngleX = getAngleKalman(KalmanAxisX,meas_angle_x,gy_valx,2.875*dt);
				}
				if (abs(kalAngleX > 90)){
					gy_valy = -gy_valy;
				}
				kalAngleY = getAngleKalman(KalmanAxisY,meas_angle_y,gy_valy,2.875*dt); 
				//tLast=tNow;
				//printf("tdelta: %ld\n", deltaT);
